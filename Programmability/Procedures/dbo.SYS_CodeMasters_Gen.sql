...
exec SYS_CodeMasters_Gen 'ASS_AMORT_DT', @p out
PRINT @p
*/
CREATE PROCEDURE [dbo].[SYS_CodeMasters_Gen]
	@p_Kind varchar(100),
	...
	ELSE
		BEGIN	
			IF EXISTS(SELECT CurValue FROM SYS_CODEMASTERS WHERE Prefix=@p_Prefix)
				UPDATE SYS_CODEMASTERS SET 	CurValue=CurValue + 1 WHERE Prefix=@p_Prefix
